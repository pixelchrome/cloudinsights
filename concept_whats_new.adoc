...
Reduce alert noise and improve monitoring precision by adding an optional second condition to any monitor. Alerts are triggered only when both conditions are met simultaneously, giving you more targeted notifications that focus on the scenarios that matter most to your environment. 

image:multi-condition_monitor_second_condition.png[adding a second condition to a monitor]
...
image:query_slideout_panel.png[illustration of a query slideout panel defaulted to the metrics and attributes tab]
