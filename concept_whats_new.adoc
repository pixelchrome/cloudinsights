...
=== Enhanced Contextual Navigation for Efficient SAN Management

Save time by leveraging Data Infrastructure Insights for operational excellence. New contextual menu links allow you to jump directly from any object to the asset landing page for device performance insights, SAN Analyzer for network topology visualization, logs for operational awareness, or Change Analyzer for configuration management.

With instant access to key data across multiple views, you gain a quicker, clearer understanding of your SAN environment and object relationships. This streamlined workflow accelerates decision-making and issue resolution, ultimately saving you time and enhancing your overall analysis capabilities.

image:contextual_menu_in_change_analysis.png[Contextual menu in Change Analysis page]

////
=== Expected Bounds in Line Widgets

When including a Line or Spline chart widget on a dashboard or landing page, you may choose to view the chart in context of the anticipated bounds for the data. You can think of this as looking for anomalies in the patterns of your data. 

DII uses seasonal data (hourly or daily) to set upper and lower bounds on where it expects the data to fall at a given time. If the data spikes above or falls below those expected bounds, the chart will highlight that.

image:expected_bounds_disk_example.png[expected bounds showing spike above, width=400]
////

////
=== Showing removed resources

image:deleted_objects_strikethrough.png[deleted objects]
////
...
Strengthen your security posture and streamline resource management with enhanced REST API Usage tracking. With API usage tracking you can see which API tokens are being used, from which IP addresses, and the volume of traffic they generate. This information, accessible to Admin users, enables you to quickly detect unusual activity patterns, identify potential threats, and optimize performance by eliminating unnecessary requests. By linking tokens to specific IP addresses and usage levels, you'll gain powerful insights into system access and usage trends, giving you the control you need to maintain a safe, efficient environment and keep operations running smoothly.

To view API Usage, navigate to *Observability > Admin > API Access* and select _View API Usage_. Note that this API is available for DII Observability APIs only; it does not apply to Workload Security or Reporting APIs.
