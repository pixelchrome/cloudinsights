...
* Port 161 and 162 (UDP) is used to access the device
...
•	Login to the device using CLI
•	Run SNMP walk
|Error: Failed to discover the USM Engine ID at xx.xx.xx.xx/161. Please check if the device is online.
|Verify that the SNMP ports 161 and 162 are open between the acquisition unit and the switch.
|===
