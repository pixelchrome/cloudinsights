...
|Permission denied when starting the Acquisition Unit Server Service|When the AU is installed on SELINUX, SE should be set to _permissive_ mode. _Enforcing_ mode is not supported. After setting SELINUX to permissive mode, restart the AU service.  

//link:https://kb.netapp.com/Cloud/BlueXP/DII/Permission_denied_when_starting_the_Cloud_Insight_Acquisition_Unit_Server_Service[Learn more].
