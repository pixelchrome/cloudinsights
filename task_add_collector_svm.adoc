...
* A Workload Security administrator must enable the Test Connectivity feature while adding a new data collector or editing an existing data collector in order for end users to use this feature. Users will see a *Test Connection* button on the add/edit collector page when the feature is enabled.
...
//Note: The Cancel button has been removed from the add/edit data collector page. The Save button has been renamed to Save Collector.

=== Test the Connection

Once the Test Connectivity feature is enabled for the user, the user can go to the add/edit collector page, enter the cluster level details (in Cluster Mode) or SVM level details (in SVM Mode), and click on the *Test Connection* button. Workload Security will then process the request and display an appropriate success or failure message.
