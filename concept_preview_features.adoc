...
|NetApp Config Advisor
|Using link:/task_dc_na_ca.html[Config Advisor] is as simple as configuring the collector and then importing the Config Advisor dashboard from the gallery. Config Advisor performs configuration validation and health checks on the whole stack of ONTAP cluster configuration to identify cabling, configuration, resiliency, availability and security issues. It is often used to validate correct system configuration at initial setup or after an upgrade.
